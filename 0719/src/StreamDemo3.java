...
/*
 * 중간연산 : filter() , map()
 * 최종연산 : count() , sum() , max() ,min() , reduce() , forEach() , average()
 */
...
public class StreamDemo3 {
	public static void main(String[] args) {
		...
		sStream.filter(str -> str.length() >= 5).forEach(s -> System.out.println(s+ "\t"));    //5글자보다 큰 
		System.out.println();
		
		List<Customer> list = new ArrayList<Customer>();
		Customer c1 = new Customer("한지민", 28); list.add(c1);
		Customer c2 = new Customer("김지민", 38); list.add(c2);
		Customer c3 = new Customer("홍지민", 48); list.add(c3);
		Customer c4 = new Customer("박지민", 58); list.add(c4);
		Stream<Customer> customerList = list.stream(); //생성 
//		customerList.map(c -> c.getName()).forEach(s -> System.out.println(s));
		customerList.map(c -> c.getInt()).forEach(s -> System.out.println(s));
	}
}
class Customer{
	private String name;
